...
interface ModalProps {
  ...
  /** Override width (e.g. 'max-w-sm', 'max-w-lg'). When set, takes precedence over size. */
  maxWidth?: string;
  /** Icon in header (e.g. <Database />). When set, header uses rounded icon box + title. */
  icon?: React.ReactNode;
  /** Footer actions (buttons). Rendered in a bar at the bottom. */
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      ...
      >
        ...
        <div className={`flex-1 overflow-y-auto min-h-0 scroll-smooth custom-scrollbar ${hideHeader ? 'p-0' : 'p-6'}`}>
          {children}
        </div>
